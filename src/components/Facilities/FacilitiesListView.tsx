...
import type { UseFacilitiesResult } from '../../hooks/useFacilities';
...
const FacilitiesListView: React.FC = () => {
  ...
  // Fetch facilities
  const { data, total, loading, error, create, update, remove } = useFacilities(
    currentPage,
    pageSize,
    searchTerm,
    sortField,
    sortDirection
  ) as UseFacilitiesResult;

  const allFields = getObjectFields('facilities').filter(f => f.id !== 'id');
    ...
    <div className="mt-4">
      {isModalOpen && (
        <FacilityDetailModal
          ...
          onSave={async (facility: Facility) => {
            if (selectedFacility) {
              await update(facility.id, facility);
            } else {
              await create(facility);
            }
            setIsModalOpen(false);
            setSelectedFacility(null);
          }}
          onDelete={
            selectedFacility
              ? async () => {
                  await remove(selectedFacility.id);
                  setIsModalOpen(false);
                  setSelectedFacility(null);
                }
              : undefined
            ...
            onClick={() => {
              const csv = [
                ...
                ...data.map(item =>
                  displayFields
                    .map(field => {
                      const value = formatCell((item as Facility)[field.id as keyof Facility], field.id);
              ...
              link.download = `facilities_export.csv`;
                ...
                return (
                  ...
                  >
                    {displayFields.map(field => (
                      ...
                      >
                        {formatCell((item as Facility)[field.id as keyof Facility], field.id)}
                      </td>
                    ))}
