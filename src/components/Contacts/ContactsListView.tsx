...
const ContactsListView: React.FC = () => {
  ...
  // Fetch contacts
  const { data, total, loading, error, create, update, remove } = useContacts(
    currentPage,
    pageSize,
    searchTerm,
    sortField,
    ...
    <div className="mt-4">
      {isModalOpen && (
        <ContactDetailModal
          ...
          onSave={async (data) => {
            // Map ContactData to Contact, ensuring accountName is present
            const contact: Contact = {
              ...selectedContact,
              ...data,
              accountName: selectedContact?.accountName ?? '', // or fetch/lookup as needed
            };
            if (selectedContact) {
          ...
          onDelete={
            selectedContact
              ? async () => {
                  await remove(selectedContact.id);
                  setIsModalOpen(false);
                  setSelectedContact(null);
                }
              : undefined
            ...
            onClick={() => {
              const csv = [
                ...
                ...data.map(item =>
                  displayFields
                    .map(field => {
                      const value = formatCell((item as Contact)[field.id as keyof Contact], field.id);
                ...
                return (
                  ...
                  >
                    {displayFields.map(field => (
                      ...
                      >
                        {formatCell((item as Contact)[field.id as keyof Contact], field.id)}
                      </td>
                    ))}
