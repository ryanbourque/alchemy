...
const DetailModal: React.FC<DetailModalProps> = ({
  ...
  const fields = getObjectFields(objectTypeId).filter(field => field.id !== 'id');
  useEffect(() => {
    setFormData({
        ...
        <div className="px-6 py-4 max-h-[calc(90vh-120px)] overflow-y-auto">
          <form onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {fields.map(field => <FormField key={field.id} field={field} value={formData[field.id]} onChange={handleChange} />)}
            </div>
